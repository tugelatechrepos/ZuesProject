...
namespace AccountBalanceManagerService.Processor
{
    ...
    public class CreateAccountBalanceListProcessor : ICreateAccountBalanceListProcessor
    {
        ...
        public IAccountAODProcessor AccountAODProcessor { get; set; }

        public IAccountOwnerProcessor AccountOwnerProcessor { get; set; }
        ...
        public CreateAccountBalanceListResponse CreateAccountBalanceList(CreateAccountBalanceListRequest Request)
        {
            ...
            _Response = new CreateAccountBalanceListResponse { AccountBalanceList = new List<AccountBalance>() };
        ...
        private void assignAccountAodList()
        {
            var accountIdList = _PaymentHistoryList.Select(x => x.AccountId).Distinct().ToList();
             var response =  AccountAODProcessor.GetAccountAODList(new GetAccountAODListRequest { AccountIdList = accountIdList });
        ...
        {
            var accountIdList = _PaymentHistoryList.Select(x => x.AccountId).Distinct().ToList();
            var response = AccountOwnerProcessor.GetAccountOwnerList(new GetAccountOwnerListRequest { AccountIdList = accountIdList });
        ...
        private void assignAccountBalanceList()
        {
            ...
            foreach (var period in periodList)
            {
                ...
                foreach(var accountGroup in accountGroupList)
                {
                    ...
                    var openingBalance = period == _CurrentPeriod ? accountOpeningBalance : (accountOpeningBalance + accountGroup.TotalPaid);
                    ...
                    accountbalance.OpeningBalance = openingBalance;
        ...
        private void assignNextPeriodAccountBalanceList()
        {
            ...
            {
                Id = x.Id,
                OpeningBalance = x.RemainingBalance,
        ...
        private decimal getOpeningBalance(Period period, int AccountId)
        {
            ...
            return accountBalance.OpeningBalance.Value;
        }
        ...
        private void persistAccountBalanceList()
        {
            ...
            do
            {
                ...
                var response = AccountBalanceProcessor.PersistAccountBalanceList(new PersistAccountBalanceListRequest
                {
